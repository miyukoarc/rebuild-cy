<template>
  <el-form ref="form" label-width="150px" label-position="left">
    ...
    <div class="text-align-center">
      <el-button v-if="detail.auditState==='TO_BE_REVIEWED'" size="small" @click="handleAudit(action='reject')">拒绝</el-button>
      <el-button v-if="detail.auditState==='TO_BE_REVIEWED'" type="primary" size="small" @click="handleAudit(action='access')">通过</el-button>
      <el-button v-if="detail.auditState!=='TO_BE_REVIEWED'" size="small" @click="handleCancel">取消</el-button>
    </div>
  </el-form>
</template>
    ...
    initData(payload) {
      this.$store
        ...
        .then(res => {
          ...
          this.tags = this.detail?.auditAddMedia[0].tagsDto ? this.grouping(this.detail?.auditAddMedia[0].tagsDto) : []
        })
        .catch(err => {
          this.$message({
            type: 'error',
